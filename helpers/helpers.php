...
function amz_wcrd_check_woocommerce_plugin()
{
	...
	if ( version_compare( wc()->version, AMZ_WCRD_MINIMUM_WOOCOMMERCE_VERSION, '<=' ) ) {
		return sprintf( __( "Please update your WooCommerce plugin. It's outdated. %s or latest required", 'amz_wcrd' ), AMZ_WCRD_MINIMUM_WOOCOMMERCE_VERSION );
	}
...
function amz_wcrd_plugin_die_message( $message )
{
	return '<p>' .
	       $message
	       . '</p> <a href="' . admin_url( 'plugins.php' ) . '">' . __( 'Go back', 'amz_wcrd' ) . '</a>';
}
