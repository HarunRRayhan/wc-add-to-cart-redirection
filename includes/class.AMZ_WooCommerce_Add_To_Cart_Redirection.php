...
final class AMZ_WooCommerce_Add_To_Cart_Redirection
{
	...
	{
		load_plugin_textdomain( 'amz_wcrd', false, AMZ_WCRD_PLUGIN_DIRNAME . '/languages' );
	}
	...
	public function settings( $settings )
	{
		...
		array_splice( $settings, 3, 0, array(
			array(
				'title'    => esc_html__( 'Redirection Link', 'amz_wcrd' ),
				'id'       => $this->_meta_name,
				'selected' => absint( get_option( 'woocommerce_checkout_page_id' ) ),
				'type'     => 'single_select_page',
				'class'    => 'wc-enhanced-select-nostd', // Means with cross icon
				'css'      => 'min-width:300px;',
				'desc_tip' => esc_html__( 'Redirection to selected page upon product added to cart', 'amz_wcrd' ),
			)
		) );
